...
parent_dir = '/Users/mac/Documents/Work/Research/Caroline/TRAPPIST_1c'
figures_dir = "/Users/mac/Documents/Work/Research/Caroline/TRAPPIST_1c/Figures for Paper"
...
class Star():
    ...
    def __init__(self, filename=None, M=0.0898*SUN['M'], R=0.1192*SUN['R'], T=2566, age=7.6*Gyr, name='TRAPPIST_1', **kwargs):
    ...
    def save(self, constants=True, params=True, folder=parent_dir, **kwargs):
        
        filename = kwargs.pop('filename', self.name)
        
        if constants:
            vs = ['name','M','R','T','age','type','M_','R_','age_','L_X']
            d = {v:[getattr(self, v)] for v in vs}
            df = pd.DataFrame.from_dict(d)
            df.to_csv(os.path.join(folder, filename + '_constants.csv'))
        
        if params:
            vs = ['dMdt_model','dMdt_boost']
            d = {v:[getattr(self, v)] for v in vs}
            df = pd.DataFrame.from_dict(d)
            df.to_csv(os.path.join(folder, filename + '_params.csv'))

        return



    def load(self, filename, constants=True, params=True, folder=parent_dir):
        ...
        if params:
            df = pd.read_csv(os.path.join(folder, filename + '_params.csv'))
            d = df.to_dict(orient='list')
            for key, val in d.items(): setattr(self, key, val[0])
...
class Planet():
    ...
    def __init__(self, filename=None, M=1.3105*EARTH['M'], R=1.097*EARTH['R'], a=0.01580*AU, name='TRAPPIST_1c', **kwargs):
    ...
    def save(self, star=True, constants=True, params=True, evolution=True, folder=parent_dir, **kwargs):
        
        filename = kwargs.pop('filename', self.name)

        if star:
            self.star.save(folder=folder, filename=filename+'_star', params=False)

        if constants:
            vs = ['name','M','R','a','M_','R_','a_',
                 'Ev','alpha','kappa','df_melt_dP','Cp','k','C1','T_surface','L_m','f_C_trapped','F_melt_plume','rho_melt','DT_sol_dep','rho_man','rho0_man','dT_dP_man',
                 'init_p_surface','init_cumulative_V_crust','init_V_crust','init_C_crust','init_Q_U238_crust','init_Q_U235_crust','init_Q_Th_crust','init_Q_K_crust']
            d = {v:[getattr(self, v)] for v in vs}
            df = pd.DataFrame.from_dict(d)
            df.to_csv(os.path.join(folder, filename + '_constants.csv'))

        if params:
            vs = ['init_C_man','mu_ref','init_T_man','HPE_','tidal_heating']
            d = {v:[getattr(self, v)] for v in vs}
            d['dMdt_model'] = self.star.dMdt_model
            d['dMdt_boost'] = self.star.dMdt_boost
            df = pd.DataFrame.from_dict(d)
            df.to_csv(os.path.join(folder, filename + '_params.csv'))
    ...
    def load(self, filename, star=True, constants=True, params=True, evolution=True, folder=parent_dir, **kwargs):
        
        i = kwargs.pop('i', None) # Use 'i' to signal retrieval from suite and give index of simulation.
        
        if star: 
            if i is None:
                self.star = Star(filename + '_star', params=False, folder=folder)
            else:
                self.star = Star('_star', params=False, folder=folder)
        ...
        if params:
            if i is None:
                df = pd.read_csv(os.path.join(folder, filename + '_params.csv'))
                d = df.to_dict(orient='list')
                self.star.dMdt_model = d.pop('dMdt_model')[0]
                self.star.dMdt_boost = d.pop('dMdt_boost')[0]
                for key, val in d.items(): setattr(self, key, val[0])
            else:
                df = pd.read_csv(os.path.join(folder, 'Summary.csv'))
                d = df.to_dict(orient='list')
                self.star.dMdt_model = d.pop('dMdt_model')[i]
                self.star.dMdt_boost = d.pop('dMdt_boost')[i]
...
def runOne(init_C_man, mu_ref, init_T_man, HPE_, tidal_heating, dMdt_model, dMdt_boost, **kwargs):
    
    p = Planet(init_C_man=init_C_man, mu_ref=mu_ref, init_T_man=init_T_man, HPE_=HPE_, tidal_heating=tidal_heating, dMdt_model=dMdt_model, dMdt_boost=dMdt_boost)
    p.evolve()
    p.save(**kwargs)

    P_surface = CS(p.t_gyr, p.P_surface)
    t_max = fmin(lambda t: -P_surface(t), p.t_gyr[np.argmax(p.P_surface)], disp=False)

    return P_surface([t_max, 5.4, 7.6, 9.8])



def suite(init_C_man, mu_ref, init_T_man, HPE_, tidal_heating, dMdt_model, dMdt_boost, name='Suite', folder=parent_dir):

    folder = os.path.join(folder, name)
    os.mkdir(folder)

    args = [init_C_man, mu_ref, init_T_man, HPE_, tidal_heating, dMdt_model, dMdt_boost]
    for i, a in enumerate(args):
        if not isinstance(a, (list,np.ndarray,tuple)): args[i] = [a]
    
    runOne(*[a[0] for a in args], filename='', folder=folder, params=False, evolution=False)

    combos = list(itertools.product(*args))
    df = pd.DataFrame(data=combos, columns=['init_C_man','mu_ref','init_T_man','HPE_','tidal_heating','dMdt_model','dMdt_boost'])
    ...
    if __name__=='__main__':
        with concurrent.futures.ProcessPoolExecutor() as executor:
            ...
            for i, combo in enumerate(combos):
                fs.append(executor.submit(runOne, *combo, filename=f'{i:07d}', star=False, constants=False, params=False, folder=folder))
...
def load(suite, q=None, index=None, init_C_man=None, mu_ref=None, init_T_man=None, HPE_=None, tidal_heating=None, dMdt_model=None, dMdt_boost=None, folder=parent_dir):

    df = pd.read_csv(os.path.join(folder, suite, 'Summary.csv'))
    
    args = [index, init_C_man, mu_ref, init_T_man, HPE_, tidal_heating, dMdt_model, dMdt_boost]
    strs = ['index', 'init_C_man', 'mu_ref', 'init_T_man', 'HPE_', 'tidal_heating', 'dMdt_model', 'dMdt_boost', 'P_surface Max', 'P_surface @ 5.4 Gyr', 'P_surface @ 7.6 Gyr', 'P_surface @ 9.8 Gyr']
    for i, a in enumerate(args):
        if not isinstance(a, (list,np.ndarray,tuple)): args[i] = [a]
    ...
    for i, a in enumerate(args):
        if a != [None]:
            ...
            for val in a:
                temp_where = temp_where | (np.array(getattr(df, strs[i])) == val)
            where = where & temp_where

    infodict = {s:np.array(getattr(df, s))[where] for s in strs}
    data = None
